...
push_all_single_arch_images() {
  ...
  parallel -j4 --tagstring "[{}]" --line-buffer \
...
push_single_arch() {
  ...
  if [[ "${last_image_nix_store_path}" == "${nix_store_path}" ]]; then
    ...
    skopeo copy --dest-compress-format="zstd:chunked" --insecure-policy --image-parallel-copies 4 --retry-times 5 nix:"${nix_store_path}" docker://"${target_image}"
